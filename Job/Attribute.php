    ...
    public function insertData()
    {
        ...
        foreach ($attributes as $index => $attribute) {
            ...
            if (ctype_digit(substr($attribute['code'], 0, 1))) {
                $this->jobExecutor->setAdditionalMessage(__('The attribute %1 was not imported because it starts with a number. Update it in Akeneo and retry.', $attribute['code']), $this->logger);
                continue;
            }
            ...
            if ($attribute['type'] == 'pim_catalog_metric' && in_array(
                ...
                if ($attribute['scopable'] || $attribute['localizable']) {
                    $this->jobExecutor->setAdditionalMessage(__('Attribute %1 is scopable or localizable please change configuration at Stores > Configuration > Catalog > Akeneo Connector > Products > Metrics management.', $attributeCode), $this->logger);
                    continue;
                }
